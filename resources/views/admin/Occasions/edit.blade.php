...
<div class="col-12">
   <div class="card">
      <div class="card-header">
         <h3 class="card-title card_title_center">  تعديل بيانات مناسبة رسمية 
         </h3>
      </div>
      <div class="card-body">
         <form action="{{ route('occasions.update',$data['id']) }}" method="post">
            ...
            <div class="col-md-12">
               <div class="form-group">
                  ...
                  <input  type="text" name="days_counter" id="days_counter" oninput="this.value=this.value.replace(/[^0-9.]/g,'');"  class="form-control" value="{{ old('days_counter',$data['days_counter']) }}"  >
                  @error('days_counter')
                  <span class="text-danger">{{ $message }}</span> 
                  @enderror
               </div>
